...
( 1057, 'introduce per-table retention policy' );


ALTER TABLE public.cluster_attribs ALTER COLUMN domain TYPE text;

INSERT INTO public.cluster_attribs (cluster_id, domain, value) VALUES
( NULL, 'metrics.retention.table=query_historical_statistics', '7 days' );
	...
	SELECT INTO pretention value FROM public.cluster_attribs
	 WHERE (cluster_id IS NULL OR cluster_id = v_cluster_id)
	   AND (domain = 'metrics.retention'
		OR domain = 'metrics.retention.table='||v_metric_table)
	 ORDER BY cluster_id NULLS LAST, value::interval ASC
	 LIMIT 1;
